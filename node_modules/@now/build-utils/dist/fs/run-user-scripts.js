...
async function runShellScript(fsPath) {
    ...
    await spawnAsync(`./${path_1.default.basename(fsPath)}`, [], destPath);
    return true;
}
...
async function getNodeVersion(destPath, minNodeVersion) {
    const { packageJson } = await scanParentDirs(destPath, true);
    const range = packageJson && packageJson.engines && packageJson.engines.node;
    if (range) {
        return node_version_1.getSupportedNodeVersion(range);
    }
    if (minNodeVersion) {
        return node_version_1.getSupportedNodeVersion(minNodeVersion, true);
    }
    return node_version_1.getSupportedNodeVersion();
}
